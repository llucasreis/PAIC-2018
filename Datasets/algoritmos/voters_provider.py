...
for i in range(1,maxOfVoters):
    ...
    for _ in range(numberOfCandidates):
        ...
        aux[candidate] = {
            'saude' : random.choice(rating),
            'seguranca' : random.choice(rating),
            'educacao' : random.choice(rating),
            'economia' : random.choice(rating),
            'cultura' : random.choice(rating),
            'tecnologia': random.choice(rating),
            'meio ambiente': random.choice(rating)
        }
